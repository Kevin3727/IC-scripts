...
geoFile = np.loadtxt('/data/user/kevin/sterile/startingTrack/data/Icecube_geometry.20110414.complete.txt')
...
def DOMDistance(self,frame,p,recoName):
    a=np.array([frame[recoName].pos.x,frame[recoName].pos.y,frame[recoName].pos.z])
    n=np.array([frame[recoName].dir.x,frame[recoName].dir.y,frame[recoName].dir.z])
...
#the time difference between hit time calculated based on the reconstructed track (recoName) and the actual hit on the DOM
def PhysicalTimeDelay(self, frame ,p , hitTime,recoName):
    ...
    uw_norm=np.linalg.norm(np.subtract(ap,aw))*1.161 #1/np.tan(0.711) #cherenkov angle is 41 deg #n=1.32
...
def TimeMask(self, frame, str_no, om_no, hitTime, recoName):
    timeDelay=PhysicalTimeDelay(self,frame,DOMPosition(self,str_no,om_no),hitTime,recoName)
...
def DepositionPosition(self,frame,p,hitTime,recoName):
    ...
    if(up_norm>250):
        return -1
    return (hitTime - (up_norm*1.32/frame[recoName].speed)) #[au_norm,up_norm*1.32, (hitTime - (frame[recoName].time + ((au_norm+(up_norm*1.32))/frame[recoName].speed))),(frame[recoName].time + ((au_norm+(up_norm*1.32))/frame[recoName].speed))]

def MuonLength(self,frame,startTime,recoName): #Only up-going muons #startTime is the muon start time excluding cascade veto
    a=np.array([frame[recoName].pos.x,frame[recoName].pos.y,frame[recoName].pos.z])
    n=np.array([frame[recoName].dir.x,frame[recoName].dir.y,frame[recoName].dir.z])
    startPos=np.add(a,np.multiply(n,((startTime - frame[recoName].time)*frame[recoName].speed)))
    safetyCounter=0
...
def BadFrameVeto(self,frame,geoframe,omkey,cascadeVetoRange,recoName): #0:good DOM 1:bad DOM
    a=np.array([frame[recoName].pos.x,frame[recoName].pos.y,frame[recoName].pos.z])
    n=np.array([frame[recoName].dir.x,frame[recoName].dir.y,frame[recoName].dir.z])
    vetoPos=np.add(a,np.multiply(n,frame['cascadeTime'].value+cascadeVetoRange-frame[recoName].time)) #NOTES: if cascade veto time !=-1, 
    domPos=np.array([geoframe.omgeo[omkey].position.x,geoframe.omgeo[omkey].position.y,geoframe.omgeo[omkey].position.z])
    if (np.dot(n,np.subtract(domPos,vetoPos))<0): #NOTE: check if the output is sensable base on the position + some all get cut
        return 1
    else:
        return 0
...
def MCStarting_polygon(self,frame):
    if(dataclasses.get_most_energetic_muon(frame['I3MCTree'])==None):
        return -1
    p=np.array([dataclasses.get_most_energetic_muon(frame['I3MCTree']).pos.x,dataclasses.get_most_energetic_muon(frame['I3MCTree']).pos.y,dataclasses.get_most_energetic_muon(frame['I3MCTree']).pos.z])
    if((p[2]<470)&(p[2]>-440)):
