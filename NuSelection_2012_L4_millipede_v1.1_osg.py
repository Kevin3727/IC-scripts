...
#METAPROJECT gsiftp://gridftp.icecube.wisc.edu/data/user/kevin/meta-projects/offline-software/trunk/build_RHEL6_full/offline-software.trunk.r132283
...
parser.add_option("-s", "--sim", dest="sim", default=1, type=int, help="0 for data, 1 for simulation, default=1")
...
cascade_service = photonics_service.I3PhotoSplineService(iceModelDir + iceModelBasename + '.abs.fits', iceModelDir + iceModelBasename + '.prob.fits', 0)
...
cascade_service = photonics_service.I3PhotoSplineService('/cvmfs/icecube.opensciencegrid.org/data/photon-tables/splines/LowEnergyCorrectedCascades_z20_a10_HE.abs.fits', '/cvmfs/icecube.opensciencegrid.org/data/photon-tables/splines/LowEnergyCorrectedCascades_z20_a10_HE.prob.fits', 0.0, 400.0)

import tarfile
tar = tarfile.open("clf_AdaBoost_90percent_10vars.tar.gz")
tar.extractall()
tar.close()

global clf4
clf4=joblib.load('clf_AdaBoost_90percent_10vars/2012_clf4_AdaBoost_90percent_10vars_v1.pkl')
...
class IceTopRemover(icetray.I3Module):
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            frame["TTPulses_recombined_noIT"] = dataclasses.I3RecoPulseSeriesMapMask(frame, "TTPulses_recombined", selector)
...
class FixTimeWindow(icetray.I3Module):
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            ...
            trigs=dataclasses.I3TriggerHierarchy.from_frame(frame,'DSTTriggers')
            #print('trig time = ', trigs[0].time)
            twindow = frame[pulses+'TimeRange']
            #print(twindow.start,twindow.stop)
            #if(trigs[0].time >twindow.stop):
            if(options.sim):
                off_pulses = dataclasses.I3RecoPulseSeriesMap.from_frame(frame,'InIcePulses')
                ...
                #print('t= ',t)
                frame[pulses+'TimeRange'] = dataclasses.I3TimeWindow(t, t + twindow.stop -twindow.start)
...
spline=np.genfromtxt('paraboloidCorrectionSpline.dat')
...
class ParabSigmaCorrCalc(icetray.I3Module):
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            ...
            frame['parabSigmaCorr']=dataclasses.I3Double(parabSigma*correctionSpline(frame['NCh'].value))
        self.PushFrame(frame)
...
class AdaBoostCalc(icetray.I3Module):
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            ...
            if(df4>-0.05):
                frame['decisionFunction_2012_10var_v1']=dataclasses.I3Double(df4)
                frame.Delete('CutL4')
        self.PushFrame(frame)
...
class ExtraWriter(icetray.I3Module):
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            frame['SPEFitParaboloid2Status']=icetray.I3Bool(frame['SPEFitParaboloid2'].fit_status==0)
        self.PushFrame(frame)
...
class MillipedeWriter(icetray.I3Module):
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            ...
            if(isStarting==1):
                if(len(f)>j_0+2):
                    cascadeEnergy_millipede = f[j_0].energy + f[j_0+1].energy + f[j_0+2].energy
                else:
                    if(len(f)>j_0+1):
                        cascadeEnergy_millipede = f[j_0].energy + f[j_0+1].energy
                    else:
                        cascadeEnergy_millipede = f[j_0].energy
            else:
                cascadeEnergy_millipede = -1
            frame['cascadeEnergy_millipede']=dataclasses.I3Double(cascadeEnergy_millipede)
            #Muon Energy
            ...
            for j in range(j_0+3,len(f)):
                if(((f[j].pos.x*f[j].pos.x)+(f[j].pos.y*f[j].pos.y))<250000):
                    if((f[j].pos.z>-500)&(f[j].pos.z<500)):
                        ...
                        muonEndPos = f[j].pos
            muonDeltax_millipede = np.linalg.norm(muonEndPos-muonStartPos) + 10. ## 10 meters
            muondEdx = muonDeltaEnergy_millipede / muonDeltax_millipede
            frame['Deltax_millipede']=dataclasses.I3Double(muonDeltax_millipede)
            frame['DeltaE_millipede']=dataclasses.I3Double(muonDeltaEnergy_millipede)
            frame['dEdx_millipede']=dataclasses.I3Double(muondEdx)
        self.PushFrame(frame)


import pickle
MuonEnergySpline = pickle.load( open( "spline_dEdx2Emu.p", "rb" ) )
    ...
    def Physics(self,frame):
        if('CutL4' not in frame.keys()):
            if(frame['cascadeEnergy_millipede'].value!=-1):
                muonEn=np.power(10,MuonEnergySpline(np.log10(frame['dEdx_millipede'].value)))
                #print(frame['dEdx_millipede'].value,type(frame['dEdx_millipede'].value))
                #print(muonEn,type(muonEn))
                frame['MuonEnergy_splined_millipede']=dataclasses.I3Double(muonEn)
                frame['NeutrinoEnergy_splined_millipede']=dataclasses.I3Double(muonEn+frame['cascadeEnergy_millipede'].value)
        self.PushFrame(frame)
